...
//------------------------------------------------------------------------------------------------------------------
    ...
    */
    ...
    Widgets.ScrollingText {
        ...
        doScroll: true
        centered: true
        visible: settingsGrid.dataNames.length != 0
    }
    ...
    MappingProperty { id: settingsBack; path: propertiesPath + ".preferencesBack";
        onValueChanged: {
            // Handle the Grid items
            if (integerEditor.value == false) settingsGrid.dataNames = []

            // Handle navigation levels
            if (thirdIndex != 0) {thirdIndex = 0; settingsGrid.currentIndex = 0}
