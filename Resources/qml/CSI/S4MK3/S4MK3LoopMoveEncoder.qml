    ...
    WiresGroup {
        ...
        WiresGroup {
            ...
            WiresGroup {
                ...
                Wire { from: "%surface%.loop_move.push"; to: TriggerPropertyAdapter { path: "app.traktor.decks." + deckId + ".track.gridmarker.set"; output: false } enabled: !shift }
                Wire { from: "%surface%.loop_move.push"; to: TriggerPropertyAdapter { path: "app.traktor.decks." + deckId + ".track.gridmarker.delete"; output: false } enabled: shift }
            }
            WiresGroup {
                enabled: zoomedEditView.value && !gridLock.value && !holdGrid.value
                Wire { from: "%surface%.loop_move"; to: "grid.offset_fine"; enabled: !shift }
