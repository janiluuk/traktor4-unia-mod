...
Module {
    ...
    WiresGroup {
        ...
        WiresGroup {
            ...
            //Hotcue Mode
            Wire { from: "%surface%.hotcue"; to: SetPropertyAdapter { path: activePadsMode.path; value: PadsMode.hotcues; color: LED.legacy(deckId) } enabled: hasTrackProperties && !shift }
            Wire { from: "%surface%.hotcue"; to: SetPropertyAdapter { path: activePadsMode.path; value: PadsMode.tonePlay; color: LED.legacy(deckId) } enabled: hasTrackProperties && (shift && shiftHotcueButton.value == 3) }

            //Loop Modes
            Wire { from: "%surface%.hotcue"; to: SetPropertyAdapter { path: activePadsMode.path; value: PadsMode.loop; color: LED.legacy(deckId) } enabled: hasDeckProperties && (shift && shiftHotcueButton.value == 0) }
            Wire { from: "%surface%.hotcue"; to: SetPropertyAdapter { path: activePadsMode.path; value: PadsMode.advancedLoop; color: LED.legacy(deckId) } enabled: hasDeckProperties && (shift && shiftHotcueButton.value == 1) }
            Wire { from: "%surface%.hotcue"; to: SetPropertyAdapter { path: activePadsMode.path; value: PadsMode.loopRoll; color: LED.legacy(deckId) } enabled: hasDeckProperties && (shift && shiftHotcueButton.value == 2) }
