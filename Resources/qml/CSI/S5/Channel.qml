...
Module
{
  ...
  Wire { from: surface_prefix + "filter_on";  to: "filterOnButton" }
  
  AppProperty { id: filterOn; path: app_prefix + "fx.on" }
  AppProperty { id: mixerFX; path: app_prefix + "fx.select" }
  MappingPropertyDescriptor { id: holdFilterOn; path: "mapping.mixer." + channel.number + ".filterOnHold"; type: MappingPropertyDescriptor.Boolean; value: false }
  ...
  ButtonScriptAdapter {
    ...
    onRelease: {
      if (holdFilterOn_countdown.running) {
        if (!channel.shift) filterOn.value = !filterOn.value
  ...
  {
    enabled: channel.shift && (fxMode.value == FxMode.TwoFxUnits)
    Wire { from: surface_prefix + "fx.assign.1"; to: ButtonScriptAdapter { onPress: { mixerFX.value == 4 ? mixerFX.value = mixerFX.value - 4 : mixerFX.value = mixerFX.value + 1 } } }
    Wire { from: surface_prefix + "fx.assign.2"; to: ButtonScriptAdapter { onPress: { mixerFX.value == 0 ? mixerFX.value = mixerFX.value + 4 : mixerFX.value = mixerFX.value - 1 } } }
  }
