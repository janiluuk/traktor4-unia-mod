...
//------------------------------------------------------------------------------------------------------------------
    ...
    AppProperty { id: mixRecorderRecording; path: "app.traktor.mix_recorder.recording"; onValueChanged: updateSettings(firstIndex, secondIndex, thirdIndex, 0, 0) }
    AppProperty { id: mixRecorderTime; path: "app.traktor.mix_recorder.recording_time"; onValueChanged: updateSettings(firstIndex, secondIndex, thirdIndex, 0, 0) }
    AppProperty { id: broadcastEnabled; path: "app.traktor.broadcast.enabled"; onValueChanged: updateSettings(firstIndex, secondIndex, thirdIndex, 0, 0) }
...
//------------------------------------------------------------------------------------------------------------------
    ...
    readonly property variant waveformColorNames: ["Default", "Red", "Dark Orange", "Light Orange", "Warm Yellow", "Yellow", "Lime", "Green", "Mint", "Cyan", "Turquoise", "Blue", "Plum",  "Violet", "Purple", "Magenta", "Fuchsia", "Prime", "CDJ-2000NXS2", "CDJ-3000", "UNIA MOD", "Customized"]
    ...
    //Traktor Settings
    if (firstIndex == 1) {
        if (secondIndex == 1) { //Transport
            if (thirdIndex == 1) {
                if (info == 1) return dataNames[index]
                else if (info == 2) return highlightText[index]
                else if (info == 3) return descriptionText[index]
                else {
                    dataNames = ["Tempo Sync (BPM)", "Beat Sync (BPM + Phase)", "Phrase Sync"]
                    highlightText = [syncMode.value == 0, syncMode.value == 1, syncMode.value == 2]
                    descriptionText = ["Match BPM only", "Match BPM and current phrase/beat phase", "Follow phrases if available"]
                }
            }
        }
            ...
            if (thirdIndex == 1) {
                if (info == 1) return dataNames[index]
                    ...
                    dataNames = ["Recording Off", "Recording On"]
                    highlightText = [!mixRecorderRecording.value, mixRecorderRecording.value]
                    descriptionText = ["Start a new mix recording. Elapsed " + formatRecordingTime(mixRecorderTime.value), "Stop and save the current recording. Elapsed " + formatRecordingTime(mixRecorderTime.value)]
                }
            }
                ...
                if (info == 1) return dataNames[index]
                    ...
                    dataNames = ["Broadcast Off", "Broadcast On"]
                    highlightText = [!broadcastEnabled.value, broadcastEnabled.value]
                    descriptionText = ["Keep broadcast streaming disabled", "Start broadcast streaming using Traktor's broadcast setup"]
                }
            }
                ...
                if (info == 1) return dataNames[index]
                    ...
                    dataNames = ["Elapsed: " + formatRecordingTime(mixRecorderTime.value)]
                    highlightText = [false]
                    descriptionText = ["Time running for the active or last mix recording"]
                }
            }
        }
    }
            ...
            if (secondIndex == 1) {
                ...
                if (thirdIndex == 1) {
                    if (info == 1) return dataNames[index]
                        ...
                        dataNames = ["Traktor", "UNIA MOD", "UNIA MOD Pro", "Prime", "CDJ-2000NXS2", "CDJ-3000"]
                        highlightText = [theme.value == 1, theme.value == 2, theme.value == 3, theme.value == 4, theme.value == 5, theme.value == 6]
                        descriptionText = ["Theme based on the Traktor PRO deck", "Theme based on Traktor PRO and Prime decks", "Redesigned UNIA MOD theme with Parallel Waveforms", "Theme which recreates Denon SC5000/SC6000 Prime decks", "Theme which recreates Pioneer CDJ-2000NXS2 decks", "Theme which recreates Pioneer CDJ-3000 decks"]
                    }
                }
                //Panels
                else if (thirdIndex == 2) {
            ...
            //Mods
                ...
                //Shift Mode
                    ...
                    if (info == 1) return dataNames[index]
                        ...
                        dataNames = ["Return to top"]
                        highlightText = [false]
                        descriptionText = ["Jump back to the first menu column"]
                    }
                }
            }
        }
    ...
    //Traktor Settings
    if (firstIndex == 1) {  //["Transport", "Mix Recorder"]
        if (secondIndex == 1) { //Transport
            if (thirdIndex == 1) {
                if (currentIndex == 0) syncMode.value = 0
                else if (currentIndex == 1) syncMode.value = 1
                else if (currentIndex == 2) syncMode.value = 2
            }
        }
        else if (secondIndex == 2) { //Mix Recorder
            if (thirdIndex == 1) {
