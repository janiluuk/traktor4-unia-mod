...
//------------------------------------------------------------------------------------------------------------------
    ...
    //MixerFXs
    MappingProperty { id: mixerFXAssigned1; path: "mapping.state.mixerFXAssigned1" }
    MappingProperty { id: mixerFXAssigned2; path: "mapping.state.mixerFXAssigned2" }
    MappingProperty { id: mixerFXAssigned3; path: "mapping.state.mixerFXAssigned3" }
    MappingProperty { id: mixerFXAssigned4; path: "mapping.state.mixerFXAssigned4" }
    readonly property variant mxrFXLabels: ["RVRB", "DLDL", "NOISE", "TIMG", "FLNG", "BRPL", "DTDL", "CRSH", "FLTR"]
    readonly property variant mxrFXNames: ["Reverb", "Dual Delay", "Noise", "Time Gater", "Flanger", "Barber Pole", "Dotted Delay", "Crush", "Filter"]
    property variant mixerFXNames: [mxrFXNames[8], mxrFXNames[mixerFXAssigned1.value], mxrFXNames[mixerFXAssigned2.value], mxrFXNames[mixerFXAssigned3.value], mxrFXNames[mixerFXAssigned4.value] ] // do not change FLTR
    property variant mixerFXLabels: [mxrFXLabels[8], mxrFXLabels[mixerFXAssigned1.value], mxrFXLabels[mixerFXAssigned2.value], mxrFXLabels[mixerFXAssigned3.value], mxrFXLabels[mixerFXAssigned4.value] ] // do not change FLTR

    //Header Fields
    MappingProperty { id: displayTopLeft; path: "mapping.state.displayTopLeft" }
    MappingProperty { id: displayTopMid; path: "mapping.state.displayTopMid" }
    MappingProperty { id: displayTopRight; path: "mapping.state.displayTopRight" }

    MappingProperty { id: displayMidLeft; path: "mapping.state.displayMidLeft" }
    MappingProperty { id: displayMidMid; path: "mapping.state.displayMidMid" }
    MappingProperty { id: displayMidRight; path: "mapping.state.displayMidRight" }

    MappingProperty { id: displayBottomLeft; path: "mapping.state.displayBottomLeft" }
    MappingProperty { id: displayBottomMid; path: "mapping.state.displayBottomMid" }
    MappingProperty { id: displayBottomRight; path: "mapping.state.displayBottomRight" }
  /*
    //App property's included in Original Code not working
    ...
    AppProperty { id: headerSettingBottomLeft; path: "app.traktor.settings.deckheader.bottom.left" }
    AppProperty { id: headerSettingBottomMid;	path: "app.traktor.settings.deckheader.bottom.mid" }
    AppProperty { id: headerSettingBottomRight; path: "app.traktor.settings.deckheader.bottom.right" }
  */
...
//------------------------------------------------------------------------------------------------------------------
    ...
    //D2 ONLY
    MappingProperty { id: d2buttons; path: "mapping.settings.d2buttons" }
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    MappingProperty { id: screenView; path: propertiesPath + ".screen_view";
        onValueChanged: {
            if (screenView.value == ScreenView.deck || screenView.value == ScreenView.browser) showSideOverlays()
        }
    }
    ...
    readonly property variant deckColors: [colors.cyan, colors.brightBlue, colors.brightBlue, colors.colorGrey232, colors.colorGrey232]
    readonly property variant darkerDeckColors: [colors.darkerColor(colors.cyan), colors.darkBlue, colors.darkBlue, colors.colorGrey72, colors.colorGrey72]
...
//--------------------------------------------------------------------------------------------------------------------
    ...
    MappingProperty { id: editMode; path: propertiesPath + ".edit_mode"; onValueChanged: { showSideOverlays() } }
    property bool isInEditMode: editMode.value == EditMode.full
    MappingProperty { id: sequencerMode; path: propertiesPath + "." + focusedDeckId + ".sequencerMode"; onValueChanged: { showSideOverlays() } }
    MappingProperty { path: propertiesPath + ".show_button_area_input"; onValueChanged: showSideOverlays() }
    property bool showButtonArea: false

    //FX Top Overlay
    TopControls {
        id: topControls
        fxUnit: topFXUnit.value //isTraktorS5 && shift ? bottomFXUnit.value : topFXUnit.value
        visible: screenView.value == ScreenView.deck
    ...
    //Performance Bottom Overlay
    BottomControls {
        id: bottomControls
        deckId: side.focusedDeckId
        sizeState: (footerHasContent.value && bottomFXPanel) ? "large" : (((screenIsSingleDeck.value && footerHasContent.value && !hideBottomPanel.value) || editMode.value == EditMode.full) ? "small" : "hide")
        visible: screenView.value == ScreenView.deck && sizeState != "hide"
        contentState: FooterPage.states[footerPage.value]
        midiId: screen.isLeftScreen ? 0 : 4
        fxUnit: bottomFXUnit.value
    }

    //Performance Faders SoftTakeover Overlay
    MappingProperty { id: softtakeoverFaders; path: propertiesPath + ".softtakeover.show_faders" }
    MappingProperty { id: softtakeoverFader1Active; path: propertiesPath + ".softtakeover.faders.1.active" }
    MappingProperty { id: softtakeoverFader2Active; path: propertiesPath + ".softtakeover.faders.2.active" }
    MappingProperty { id: softtakeoverFader3Active; path: propertiesPath + ".softtakeover.faders.3.active" }
    MappingProperty { id: softtakeoverFader4Active; path: propertiesPath + ".softtakeover.faders.4.active" }
    property bool showFaders: !directThru.value && (side.focusedDeck().deckType == DeckType.Remix || side.focusedDeck().deckType == DeckType.Stem) && (softtakeoverFader1Active.value || softtakeoverFader2Active.value || softtakeoverFader3Active.value || softtakeoverFader4Active.value)
    onShowFadersChanged: if (showFaders) showFadersTimer.restart()
    Timer { id: showFadersTimer; interval: sideButtonsTimer.value }
    SoftTakeoverFaders {
        id: faders
        anchors.left: parent.left
        anchors.right: parent.right
        anchors.bottom: parent.bottom
        visible: screen.flavor != ScreenFlavor.S5 && (softtakeoverFaders.value || showFadersTimer.running)
    }
    ...
    ButtonArea {
        ...
        state: ""
        visibleState: showButtonArea ? "show" : "hide"
        contentState: "TextArea"
        textAngle: -90
        scrollPosition: samplePage.value
        isTopHighlighted: editMode.value == EditMode.full && gridLock.value
        isBottomHighlighted: editMode.value == EditMode.full && isTrackTick.value
    }
    ...
    ButtonArea {
        ...
        state: "hide"
        visibleState: (showButtonArea || showRightAreaStemShifted) ? "show" : "hide"
        textAngle: 90
        scrollPosition: samplePage.value
        isTopHighlighted: editMode.value == EditMode.full && gridLock.value && (screen.flavor != ScreenFlavor.S5 || (screen.flavor == ScreenFlavor.S5 && !shift))
        isBottomHighlighted: editMode.value == EditMode.full && gridLock.value
    }

    property bool showRightAreaStemShifted: false
    onShiftChanged: {
        if (focusedDeckType == DeckType.Stem && shift && editMode.value != EditMode.full) {
            showRightAreaStemShifted = true
            showButtonAreaTimer.restart()
            rightOverlay.visible = true
            rightOverlay.contentState = shift ? "TextArea" : "Magnifiers"
            rightOverlay.topText = "TRACK"
            rightOverlay.bottomText = "STEM"
        }
    }

    //Functions for updating the left & right overlays content
    onFocusedDeckIdChanged: showSideOverlays()
    onFocusedDeckTypeChanged: showSideOverlays()

    Timer { id: showButtonAreaTimer; interval: sideButtonsTimer.value;
        onTriggered: {
            if (editMode.value != EditMode.full){
                showButtonArea = false
            }
            showRightAreaStemShifted = false
        }
    }

    function showSideOverlays() {
        if (directThru.value || side.focusedDeck().deckType == DeckType.Live) { showButtonArea = false }
        else { showButtonArea = true; showButtonAreaTimer.restart() }

        if (screenView.value == ScreenView.deck && editMode.value == EditMode.disabled) {
            if (side.focusedDeck().deckType == DeckType.Track) {
                leftOverlay.visible = true
                leftOverlay.contentState = "TextArea"
                leftOverlay.topText = "BPM"
                leftOverlay.bottomText = "KEY"

                rightOverlay.contentState = "Magnifiers"
            }
            else if (side.focusedDeck().deckType == DeckType.Stem) {
                leftOverlay.visible = true
                leftOverlay.contentState = "TextArea"
                leftOverlay.topText = "BPM"
                leftOverlay.bottomText = "KEY"
            ...
            else if (side.focusedDeck().deckType == DeckType.Remix) {
                leftOverlay.visible = true
                leftOverlay.contentState = "TextArea"
                leftOverlay.topText = "BPM"
                leftOverlay.bottomText = sequencerMode.value ? "SWING" : "QUANTIZE"

                rightOverlay.contentState = sequencerMode.value ? "TextArea" : "ScrollBar"
                rightOverlay.topText = "1-8"
                rightOverlay.bottomText = "9-16"
            }
        }
        else if (screenView.value == ScreenView.deck && editMode.value == EditMode.full) {
            leftOverlay.visible = true
            leftOverlay.contentState = "EditIcons"
            leftOverlay.topText = "LOCK"
            leftOverlay.bottomText = "TICK"

            rightOverlay.contentState = "TextArea"
            rightOverlay.topText = (screen.flavor != ScreenFlavor.S5 || (screen.flavor == ScreenFlavor.S5 && !shift)) ? "AUTO" : "ZOOM IN"
            rightOverlay.bottomText = (screen.flavor != ScreenFlavor.S5 || (screen.flavor == ScreenFlavor.S5 && !shift)) ? "RESET" : "ZOOM OUT"
        }
        else if (screenView.value == ScreenView.browser) {
            leftOverlay.visible = false

            rightOverlay.contentState = "TextArea"
            rightOverlay.topText = "+ PREP"
            rightOverlay.bottomText = "PREP. LIST"
        }
    }
}
