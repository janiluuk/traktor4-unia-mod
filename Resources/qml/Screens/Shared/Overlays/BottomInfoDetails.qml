...
Rectangle {
  ...
  color: isTraktorS5 && slotState && (performanceEncoderControls.value != column && column != 0) ? colors.darkerColor(backgroundColor, 0.5) : backgroundColor
...
//------------------------------------------------------------------------------------------------------------------
  ...
  readonly property string finalLabel: column == 0 ? (fxType.value == FxType.Single ? fxSelect1.description : fxType.value == FxType.Group ? "DRY/WET" : "Volume") : parameterName.description
  ...
  states: [
    ...
    State {
        ...
        PropertyChanges { target: parameterDisplayText; toPercent: false }
        PropertyChanges { target: parameterDisplayText; font.pixelSize: fonts.largeValueFontSize }
        PropertyChanges { target: activeButton; visible: column != 0 || fxType.value != FxType.Group }
        PropertyChanges { target: activeButtonText; text: column == 0 ? "ON" : fxButtonName.value }
        PropertyChanges { target: bottomInfoDetails; isEnabled: column == 0 ? (fxType.value == FxType.Group || fxSelect1.value) : (fxColumnSelect.value || (fxType.value == FxType.Single && fxSelect1.value)) }
    },
    State {
        name: "FILTER"
